    ...
    def draw(self, surface):
        for i in range(4):
            surface.blit( self.cells[i], 
              ((self.x + self.coordDict[self.rot][i][0]) * Mino.SIZE, 
              (self.y + self.coordDict[self.rot][i][1]) * Mino.SIZE) )
...
class Playfield(pg.sprite.Sprite):
    ...
    def __init__(self):
        # Instance Variables
        self.surf = pg.Surface((self.WIDTH * Mino.SIZE + 10, self.HEIGHT * Mino.SIZE + 5))
        self.field = self.surf.subsurface((5, 0, self.WIDTH * Mino.SIZE, self.HEIGHT * Mino.SIZE))
